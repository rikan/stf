...
// Export
module.exports.login = function (options, username, password, uidNumber) {
  function tryConnect() {
    var resolver = Promise.defer()
    var client = ldap.createClient({
      url: options.url, timeout: options.timeout, maxConnections: 1
    })
  ...
  function tryFind(client) {
    var resolver = Promise.defer()
    var filters = [new ldap.EqualityFilter({
...
module.exports.email = function (user) {
  return user.uidNumber || user.mail || user.email || user.userPrincipalName
}
